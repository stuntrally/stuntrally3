...
void main()
{
	...
	uv5.y -= 0.5;  uv5.y *= 2.0;
                                                 uv4.x -= 0.5;  uv5.x -= 0.5;
	uv1.x *= 2.0;  uv2.x *= 2.0;  uv3.x *= 2.0;  uv4.x *= 2.0;	uv5.x *= 2.0;

	fragColour =
		uv.x < 0.5 ?
		(uv.y < t1 ? texture( vkSampler2D( tex1, sampler1 ), uv1 ) :
		 uv.y < t2 ? texture( vkSampler2D( tex2, sampler2 ), uv2 ) :
		             texture( vkSampler2D( tex3, sampler3 ), uv3 )) :
		(uv.y < 0.5
		 ? texture( vkSampler2D( tex4, sampler4 ), uv4 )
		 : texture( vkSampler2D( tex5, sampler5 ), uv5 ));
}
