...
// sv= &svWaterSkip;	sv->Init("WaterSkip",	&pSet->g.water_skip,	0,20, 2.f);  sv->DefaultI(0);
	...
	sv= &svVolDynamic;	sv->Init("VolDynamic",  &pSet->s.vol_dynamic, 0.f, 2.0f);  sv->DefaultF(1.f);
	ck= &ckSndDynamic;	ck->Init("SndDynamic",  &pSet->s.snd_dynamic);

#ifndef SR_EDITOR  // game
	sv= &svVolEngine;	sv->Init("VolEngine",	&pSet->s.vol_engine, 0.f, 1.4f);  sv->DefaultF(0.58f);
	sv= &svVolTurbo;	sv->Init("VolTurbo",	&pSet->s.vol_turbo, 0.f, 3.0f);  sv->DefaultF(1.f);
	sv= &svVolGear;		sv->Init("VolGear",		&pSet->s.vol_gear, 0.f, 1.6f);  sv->DefaultF(0.4f);
