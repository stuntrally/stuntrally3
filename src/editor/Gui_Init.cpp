...
void CGui::InitGui() 
{
	...
	Load("Editor_Main");   Load("Editor_Track");
	Load("Editor");        Load("Editor_Objects");
	...
	app->mWndEdit = fWnd("EditorWnd");  app->mWndEdObj = fWnd("EdObjWnd");
	...
	app->mWndFields =  fWnd("FieldsWnd");  app->mWndFields->setPosition(0,y);
	...
	vSubTabsEdObj.clear();
	for (u=0; u < app->mTabsEdObj->getItemCount(); ++u)
	{
		sub = gcom->FindSubTab(app->mTabsEdObj->getItemAt(u));
		vSubTabsEdObj.push_back(sub);
	}
	...
	{	String s = toStr(i);
		...
		if (i < FLD_TXT)  fldTxt[i]= fTxt("fldTxt"+s);
	}
	...
	{	String sn = "RainRate" + toStr(i);
		sv= &svRainRate[i];  sv->Init(sn,	&sc->rainEmit[i],   0.f,90000.f, 2.5f);  sv->DefaultF(1000.f);
	}
	//  🌫️ Fog
	...
	sv= &svRoR_Friction;	sv->Init("RoR_Friction", &pSet->rorFriction,  -0.4f,0.2f, 1.f, 2,4);  sv->DefaultF(-0.1f);
	...
	sv= &svRoR_RoadHadd;	sv->Init("RoR_RoadHadd",	&ror.roadHadd,  -2.f, 1.f, 1.f, 2,4);  sv->DefaultF(-0.45f);
	sv= &svRoR_WallX;		sv->Init("RoR_WallX",		&ror.wallX,		0.f, 10.f, 2.f, 2,4);  sv->DefaultF(0.6f);
	sv= &svRoR_WallY;		sv->Init("RoR_WallY",		&ror.wallY,		0.f, 10.f, 2.f, 2,4);  sv->DefaultF(1.f);
