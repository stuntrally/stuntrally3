...
void BrushesIni::SerBrush(bool wr, CONFIGFILE & cf, const std::string s, BrushSet& p)
{
	...
	Param(cf,wr, s+"size", p.Size);  Param(cf,wr, s+"int",  p.Intens);  Param(cf,wr, s+"pow",  p.Pow);
	Param(cf,wr, s+"freq", p.Freq);  Param(cf,wr, s+"nofs", p.NOfs);    Param(cf,wr, s+"oct",  p.Oct);
	Param(cf,wr, s+"shape",p.shape);
	Param(cf,wr, s+"filt", p.Filter);  Param(cf,wr, s+"hset", p.HSet);
...
const static BrushSet brSetsDef[brSetsNum] = {
...
void BrushesIni::LoadDef()
{
	v.clear();
	for (int i=0; i < brSetsNum; ++i)
		v.push_back(brSetsDef[i]);
}
