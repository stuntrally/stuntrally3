...
void App::CreateGraphs()
{
	...
	case Gh_Tires4Edit:  //  ⚫📉4 all tires pacejka vis,edit
		for (i=0; i < nWh * 2; ++i)
		{
			gv = new GraphView(scm,mWindow,mGui);
			c = 2;  bool b = i >= nWh;
...
void App::GraphsNewVals()				// Game
{
	...
	if (gsi >= 5)
	{
		...
		graphs[1]->UpdTitle("\nbuf: "+toStr(snd->buffers_used_max)+" /"+toStr(pSet->s.cnt_buffers)+"\n"+
	...
	if (gsi >= 2)
	{
		...
		const float fps = st->getFps(); //getAvgFps();
...
{	
	...
	case Gh_BulletHit:  //  ✨ bullet hit
		if (gsi >= 6)
		{	// force, normvel,  sndnum, scrap, screech
